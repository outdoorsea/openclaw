...
`;
      ...
      scriptContent = `#!/bin/sh
# Standalone restart script — survives parent process termination.
# Wait briefly to ensure the current launchd service tree has exited.
sleep 1
# Try kickstart first so a healthy LaunchAgent can restart without unloading.
# If that fails (for example after a prior bootout removed the service), clear
# any persisted disabled state, then bootstrap and kickstart again.
